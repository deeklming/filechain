...
Filechain(파일체인)은 Blockchain(블록체인)을 기반으로 하고 저비용 고사양을 목표로 한다.   
네트워크 기능을 필요로 하고 데스크톱, 모바일, IoT, 임베디드 환경 등에서 사용가능 하도록 목표로 한다.
...
```
    ...
    failure: 2 or less
```
네트워크는 메시형으로 구성하고 서버는 현재 실행중인 모든 클라이언트 프로그램 정보를 필요로 한다.   
